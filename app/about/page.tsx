...
function Spotlight() {
  ...
  // ✅ useTransform dipanggil SEBELUM conditional return
...
function ParallaxLayer({ children, offset = 80, className, style }: {
  children: React.ReactNode
  offset?: number
  className?: string
  style?: React.CSSProperties
  ...
  const y = useTransform(scrollYProgress, [0, 1], [`-${offset / 2}px`, `${offset / 2}px`])
...
export default function AboutPage() {
  ...
  const bioImgY  = useTransform(bioScroll, [0, 1], ["-8%", "8%"])
  const bioTextX = useTransform(bioScroll, [0, 1], ["30px", "0px"])

  // GSAP: Hero title char reveal
  useGSAP(() => {
  ...
  // GSAP: Bio photo clip reveal
  useGSAP(() => {
    const img = bioRef.current?.querySelector(".bio-photo")
    if (!img) return
    gsap.fromTo(img,
  ...
  return (
    <main style={{ background: "var(--bg)", color: "var(--text-primary)" }}>
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <motion.div
          style={{
            y: useTransform(heroScroll, [0, 1], ["0px", "80px"]),
        ...
        <motion.div
          style={{
            y: useTransform(heroScroll, [0, 1], ["0px", "-50px"]),
      ...
      {/* ── BIO ──────────────────────────────────────────────── */}
      <section ref={bioRef} className="px-6 md:px-16 py-24 md:py-32 max-w-6xl mx-auto">
        <div className="grid md:grid-cols-2 gap-16 md:gap-24 items-center">

          {/* Foto */}
          <div className="relative">
            <motion.div style={{ y: bioImgY }} className="relative z-10">
              <motion.div
                className="absolute -top-4 -left-4 w-full h-full rounded-sm"
                style={{ border: "1px solid rgba(34,179,208,0.2)" }}
                animate={{ rotate: [0, 0.5, 0, -0.5, 0] }}
                transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
              />
              <motion.div
                className="absolute -bottom-4 -right-4 w-full h-full rounded-sm"
                style={{ border: "1px solid rgba(211,179,102,0.15)" }}
                animate={{ rotate: [0, -0.5, 0, 0.5, 0] }}
                transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
              />

              <TiltCard className="bio-photo group">
                <Image
                  src="https://picsum.photos/seed/photographer-portrait/600/750"
                  alt="Shobiryne" width={600} height={750}
                  className="w-full h-auto object-cover rounded-sm"
                  style={{ filter: "saturate(0.82)" }}
                />
                {/* Scan line hover */}
                <motion.div
                  className="absolute inset-0 pointer-events-none rounded-sm opacity-0 group-hover:opacity-100"
                  transition={{ opacity: { duration: 0.3 } }}
                  animate={{ backgroundPosition: ["0 -100%", "0 200%"] }}
                  style={{
                    background: "linear-gradient(to bottom, transparent 40%, rgba(34,179,208,0.06) 50%, transparent 60%)",
              ...
              <motion.div
                ...
                className="absolute -bottom-6 -right-6 z-20 px-5 py-3 rounded-sm cursor-default"
                style={{
                  background: "rgba(14,40,48,0.92)",
                  border: "1px solid rgba(34,179,208,0.2)",
                  backdropFilter: "blur(12px)",
                }}
              >
                <p className="text-xs tracking-[0.3em] uppercase" style={{ color: "var(--cyan)", opacity: 0.7 }}>Est.</p>
                <p className="font-light italic"
                  style={{ fontFamily: "var(--font-display)", fontSize: "1.8rem", color: "var(--gold)", lineHeight: 1 }}>
                  2018
                </p>
              </motion.div>
            </motion.div>
          ...
          >
            <div>
              <motion.span
                ...
                viewport={{ once: true }}
                className="block text-xs tracking-[0.45em] uppercase mb-3 overflow-hidden whitespace-nowrap"
                style={{ color: "var(--cyan)", opacity: 0.7 }}
              >
                Tentang Saya
              </motion.span>
              <h2 className="font-light italic mb-6"
                style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", color: "var(--gold)" }}>
                Shobiryne
              </h2>
            </div>

            {[
              <>Saya adalah fotografer dan videografer berbasis di Indonesia yang berfokus pada estetika sinematik. Setiap karya yang saya buat berangkat dari satu pertanyaan sederhana: <em style={{ color: "var(--text-primary)" }}>bagaimana cahaya ini terasa?</em></>,
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        className="w-full max-w-5xl mx-auto h-px my-0 origin-left"
        style={{ background: "linear-gradient(to right, transparent, var(--pink), transparent)", opacity: 0.2 }}
      />
      ...
      <section className="px-6 md:px-16 py-28 md:py-40 relative overflow-hidden">
        ...
        {/* BG parallax — ✅ position relative + minHeight fix */}
        <ParallaxLayer
          offset={60}
